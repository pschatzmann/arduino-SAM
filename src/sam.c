#include <stdio.h> // define SAM_LOG()
...
// Insert at WX or YX following, copying the stress
		...
		if (SAM_DEBUG) if (A==20) SAM_LOG("RULE: insert WX following diphtong NOT ending in IY sound");
		if (SAM_DEBUG) if (A==21) SAM_LOG("RULE: insert YX following diphtong ending in IY sound");
...
// Skip this rule if phoneme is not UL
		...
		if (SAM_DEBUG) SAM_LOG("RULE: UL -> AX L");
...
// Skip rule if phoneme != UM
		...
		if (SAM_DEBUG) SAM_LOG("RULE: UM -> AX M");
		goto pos41466;
pos41495:
...
// Skip rule if phoneme != UN
		...
		if (SAM_DEBUG) SAM_LOG("RULE: UN -> AX N");
		goto pos41466;
pos41503:
...
// Insert a glottal stop and move forward
							if (SAM_DEBUG) SAM_LOG("RULE: Insert glottal stop between two stressed vowels with space between them");
							// 31 = 'Q'
							Insert(X, 31, mem59, 0);
							pos++;
							continue;
...
// Look at prior phoneme
		...
		{
// Change T to CH
			if (SAM_DEBUG) SAM_LOG("RULE: T R -> CH R");
...
// Change D to J
			...
			if (SAM_DEBUG) SAM_LOG("RULE: D R -> J R");
			goto pos41788;
		}
...
// If vowel flag is set change R to RX
		A = flags[A] & 128;
		if (SAM_DEBUG) SAM_LOG("RULE: R -> RX");
...
// Prior phoneme has VOWEL flag set, so change L to LX and move to next phoneme
			if (SAM_DEBUG) SAM_LOG("RULE: <VOWEL> L -> <VOWEL> LX");
...
// Replace S with Z and move on
			if (SAM_DEBUG) SAM_LOG("RULE: G S -> G Z");
...
// VOWELS AND DIPHTONGS ENDING WITH IY SOUND flag set?
				A = flags[Y] & 32;
				if (SAM_DEBUG) if (A==0) SAM_LOG("RULE: K <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> KX <VOWEL OR DIPHTONG NOT ENDING WITH IY>");
// Replace with KX
...
// replace G with GX and continue processing next phoneme
			if (SAM_DEBUG) SAM_LOG("RULE: G <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> GX <VOWEL OR DIPHTONG NOT ENDING WITH IY>");
...
// If not set, continue processing next phoneme
			if (A == 0) {pos++; continue;}
			if (SAM_DEBUG) SAM_LOG("RULE: <ALVEOLAR> UW -> <ALVEOLAR> UX");
			phonemeindex[X] = 16;
			pos++;
			continue;
		}
...
// Example: CHEW
		...
		{
			//        pos41783:
			if (SAM_DEBUG) SAM_LOG("CH -> CH CH+1");
...
//pos41856:
// Set phonemes to DX
		if (SAM_DEBUG) SAM_LOG("RULE: Soften T or D following vowel or ER and preceding a pause -> DX");
...
// Is next phoneme a vowel or ER?
				A = flags[A] & 128;
			if (SAM_DEBUG) if (A != 0) SAM_LOG("RULE: Soften T or D following vowel or ER and preceding a pause -> DX");
...
pos48644:
		...
		do
		{
			...
			if(((flags2[index] & 32) == 0) || ((flags[index] & 4) != 0))     //nochmal �berpr�fen
			{
				...
				A = (A >> 1) + A + 1;
if (SAM_DEBUG) SAM_LOG("RULE: Lengthen <FRICATIVE> or <VOICED> between <VOWEL> and <PUNCTUATION> by 1.5");
if (SAM_DEBUG) SAM_LOG("PRE");
if (SAM_DEBUG) SAM_LOG("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);

				phonemeLength[X] = A;

if (SAM_DEBUG) SAM_LOG("POST");
if (SAM_DEBUG) SAM_LOG("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
...
if (SAM_DEBUG) SAM_LOG("RULE: <VOWEL> <RX | LX> <CONSONANT> - decrease length by 1");
if (SAM_DEBUG) SAM_LOG("PRE");
...
if (SAM_DEBUG) SAM_LOG("RULE: <VOWEL> <UNVOICED PLOSIVE> - decrease vowel by 1/8th");
if (SAM_DEBUG) SAM_LOG("PRE");
if (SAM_DEBUG) SAM_LOG("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]],  phonemeLength[X]);
...
if (SAM_DEBUG) SAM_LOG("RULE: <VOWEL> <VOICED CONSONANT> - increase vowel by 1/2 + 1");
if (SAM_DEBUG) SAM_LOG("PRE");
...
//pos48821:
        ...
        if((flags2[index] & 8) != 0)
        {
            ...
            {
if (SAM_DEBUG) SAM_LOG("RULE: <NASAL> <STOP CONSONANT> - set nasal = 5, consonant = 6");
if (SAM_DEBUG) SAM_LOG("POST");
if (SAM_DEBUG) SAM_LOG("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
if (SAM_DEBUG) SAM_LOG("phoneme %d (%c%c) length %d\n", X-1, signInputTable1[phonemeindex[X-1]], signInputTable2[phonemeindex[X-1]], phonemeLength[X-1]);
...
if (SAM_DEBUG) SAM_LOG("RULE: <UNVOICED STOP CONSONANT> {optional silence} <STOP CONSONANT> - shorten both to 1/2 + 1");
if (SAM_DEBUG) SAM_LOG("PRE");
if (SAM_DEBUG) SAM_LOG("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
if (SAM_DEBUG) SAM_LOG("phoneme %d (%c%c) length %d\n", X-1, signInputTable1[phonemeindex[X-1]], signInputTable2[phonemeindex[X-1]], phonemeLength[X-1]);
...
if (SAM_DEBUG) SAM_LOG("RULE: <LIQUID CONSONANT> <DIPHTONG> - decrease by 2");
if (SAM_DEBUG) SAM_LOG("PRE");
if (SAM_DEBUG) SAM_LOG("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
